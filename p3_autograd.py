...
print(z)
z.backward() #method which generates gradient of z with resepect to x == dz/dx:
# in .bakcward() method here we dont need to use argument inside, cause z is scala value
...
print(x.grad)
print('* GRAD - it also produces Vector Jacobian products in background - chain rule - later it is J*v, so we need to multiply (J * vector) \n')
...
print('\n\nEXAMPLE 4 - WHENEVER WE CALL THE BACKGROUND FUNCTION THEN THE GRADIENT FOR THIS TENSOR WILL BE ACCUMULATED IN THE GRAD ATR|IBUTE, VALUES WILL BE SUMMED UP ')
print('TRAINING EXAMPLE with some weights: ')
weights = torch.ones(4, requires_grad=True)
for epoch in range(1): #make at first only one iteration, and then check what will happen if we make 2 or more iterations
